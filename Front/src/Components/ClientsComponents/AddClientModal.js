...
import { AddClient } from "../../actions/clientsAction";
...
const AddClientModal = (props) => {
  ...
  const result = useSelector((states) => states.creteClient);
  const { loadingadd, successadd, clientAdd, erroradd } = result;
  if (successadd) {
    props.Swal.fire({
    ...
    dispatch({ type: POST_CLIENT_RESET });
    props.add(clientAdd);
  }
  if (erroradd) {
  ...
  const handleData = async (data) => {
    dispatch({ type: GET_CLIENTS_RESET });
        ...
        <form onSubmit={handleSubmit(handleData)}>
          <Modal.Header closeButton>
            <Modal.Title id="contained-modal-title-vcenter">
              <i className="fas fa-user-plus" /> إضافة زبون
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
